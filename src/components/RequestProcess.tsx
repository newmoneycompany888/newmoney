...
export const RequestProcess: FC<IRequestProcessProps> = () => {
  return (
    <div className="text-center">
      <h2 className="text-primary dark:text-white text-xl md:text-2xl lg:text-3xl font-extrabold mb-0.5 md:mb-1">ขั้นตอนการขอสินเชื่อ</h2>
      <p className="text-gray-700 dark:text-gray-400">สำหรับผู้ที่สนใจยื่นกู้สินเชื่อกับเรา มีขั้นตอนดำเนินการดังต่อไปนี้</p>
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mt-4 lg:mt-6">
        {REQUEST_PROCESSES.map((requestProcess, index) => (
          <div key={`request-process-${index}`}>
            <div className="bg-white rounded-md shadow-md mb-2 sm:mb-4 p-0.5">
              <img src={requestProcess.image} alt="request-process" className="w-full aspect-[3/2] object-cover rounded-md" />
            </div>
            <h4 className="dark:text-white text-xl font-bold mb-1">{requestProcess.title}</h4>
